...
public class LanguageSubCommand extends SubCommand<ICmdSender> {
    @Override
    public void run(final ICmdSender sender, final String[] args, final String[] args2) {
        final IPlayerCmdSender pSender = (IPlayerCmdSender) sender;
        if (args.length <= 1) {
            ...
            LibrarySQLConfig.singleton().getTransactionManager().required(() -> {
                MUserEntity entity = MUserRepo.selectById(pSender.getUniqueId());
                if (entity == null) {
                    entity = new MUserEntity();
                    entity.setPlayerUuid(pSender.getUniqueId());
                }
